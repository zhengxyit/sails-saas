            ...
            async.auto({
                'config/*': function loadOtherConfigFiles(cb) {
                    buildDictionary.aggregate({
                        ...
                        exclude: ['locales'].concat(_.map(sails.config.moduleloader.configExt, function (item) {
                            return 'local.' + item;
                        })),
            ...
            buildDictionary.optional({
                ...
                buildDictionary.optional({
                    ...
                    if (sails.config.models.migrate != 'safe') {
                        console.log("sails.config.models.migrate is [" + sails.config.models.migrate + '] exit');
                        process.exit(1);
                    }
