...
var actionUtil = {
    ...
    parseCriteria: function (req) {
        ...
        if(Model.multitenant){
            where.or = [{tenantId: this.parseTenantId(req)},{tenantId: '*'}];
        }
    ...
    parseValues: function (req) {
        ...
        var Model = this.parseModel(req);
        if(Model.multitenant) {
            var tenantId = this.parseTenantId(req);
        }
    ...
    isIpAddress: function (req) {
        var host = req.host + "";
        if (host.match(/^([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/)) {
            return true;
        } else {
            return false;
        }
