...
var actionUtil = {
     ...
     * @param  {Request} req
     * @param  {boolean} isEdit 如果是编辑，不查询*的，Saas支持修改
     * @return {Query}
     */
    populateRequest: function (query, req, isEdit) {
        ...
        var Model = this.parseModel(req);
        if (Model.multitenant) {
            if (isEdit) {
                query._criteria.where.tenantId = {like: '%,' + this.parseTenantId(req) + ',%'};
            } else {
                query._criteria.where.or = [
                    {tenantId: {like: '%,' + this.parseTenantId(req) + ',%'}},
                    {tenantId: {like: '%,*,%'}}
                ]
            }
        }
     ...
     *
     * @param  {Request} req
     * @param  {Request} isCreate
     * @return {Object}
     */
    parseValues: function (req, isCreate) {
        ...
        var Model = this.parseModel(req);
        var tenantId;
        if (isCreate && Model.multitenant) {
            tenantId = ',' + this.parseTenantId(req) + ',';
        }
    ...
    isIpAddress: function (req) {
        var host = req.host + "";
        if (host == 'localhost' || host.match(/^([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/)) {
            return true;
        } else {
            return false;
        }
    ...
    parseTenantId: function (req) {
        if (this.isIpAddress(req)) {
            // TODO 暂时
            return "*";
        }
        if (req._sails.config.blueprints.domain) {
            var host = req.host + "";
            var domain = req._sails.config.blueprints.domain.replace(/\*/, '');
            var str = host.replace(eval('/' + domain + '/'), '');
            return str;
        } else {
            return "*";
        }
