...
module.exports = function updateOneRecord(req, res) {
    ...
    if (!pk) {
        ...
        var reqBody = req.body;// actionUtil.parseValues(req);
        Model.update(reqParams, reqBody).exec(function updated(err, updatedCount) {
            if (err) return res.negotiate(err);
            res.json({result:updatedCount.length});
        });
    }else{
        ...
        query = actionUtil.populateRequest(query, req, true);
